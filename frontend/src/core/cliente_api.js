export async function obtenerDatosNasa(latitud, longitud) {
    try {
        const respuesta = await fetch(`/api/v1/nasa-data?lat=${latitud}&lon=${longitud}`);
        if (!respuesta.ok) {
        ...
        console.warn("No se pudo conectar al Backend para NASA POWER. Usando datos estimados según coordenadas.", error);
        const irradianciaEstimada = parseFloat((5.2 + Math.cos((latitud * Math.PI) / 180) * 1.5).toFixed(2));
        return {
            latitud,
            longitud,
...
export async function detectarPanelesSolares(archivoImagen, ctxCanvas, ancho = 640, alto = 480) {
    ...
    try {
        ...
        if (respuesta.ok) {
            ...
            return {
                ...
                cajasDelimitadoras: (datos.cajas_delimitadoras || []).map((c) => ({
                    xMin: c.x_min,
                    yMin: c.y_min,
                    xMax: c.x_max,
                    yMax: c.y_max,
...
export async function obtenerImagenSatelitalReal(latitud, longitud) {
    try {
